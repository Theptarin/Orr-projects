...
/* aliases */
    ...
    public function extractPermission($what, $permission = false, $table = false) {
        ...
        if (is_bool($permission)) {
            ...
            if (!$table) {
                echo "You need to pass a table to 'this->login_model->extractPermission()' as third parameter to use the current logged user permissions!";
                die;
            }
        ...
        if ($what == 2 || $what == "RL" || $what == "readlist")
            $return = $permission[1];
        if ($what == 3 || $what == "RS" || $what == "readsingle")
            $return = $permission[2];
        if ($what == 4 || $what == "A" || $what == "add")
            $return = $permission[3];
        if ($what == 5 || $what == "E" || $what == "edit")
            $return = $permission[4];
        if ($what == 6 || $what == "D" || $what == "delete")
            $return = $permission[5];
        return $return;
    }
    ...
    public function check($crud, $author = false) {
        $state = unserialize(
                preg_replace(
                        '/^O:\d+:"[^"]++"/', 'O:' . strlen("portapipe") . ':"portapipe"', serialize($crud)
                )
        );
        ...
        if (!$this->extractPermission("RL", false, $table))
            $crud->unset_list();
        if (!$this->extractPermission("RS", false, $table))
            $crud->unset_read();
        if (!$this->extractPermission("D", false, $table))
            $crud->unset_delete();
        if (!$this->extractPermission("A", false, $table))
            $crud->unset_add();
        if (!$this->extractPermission("E", false, $table))
            $crud->unset_edit();
        if (!$this->extractPermission("D", false, $table))
            $crud->unset_delete();

        if ($author)
            $crud->where($author, $this->id());
